...
impl Scope {
    ...
    pub fn resolve(&self, path: &NodePath) -> Option<crate::ref_resolve::ChainValue> {
        let (head, tail) = path.0.split_first()?;
        let recs = crate::ref_resolve::head_records(
            self,
            crate::ref_resolve::RefKind::Qualified(head.kind),
            head.id.as_str(),
        );
        // Convert tail segments to a `Hop` list (children use kind-blind id keys; Regex means field projection).
        let hops: Vec<dir_lint_expr::Hop> = tail
            .iter()
            .map(|seg| match seg.kind {
                NodeKind::Regex => dir_lint_expr::Hop::Regex(seg.id.0.clone()),
                _ => dir_lint_expr::Hop::Dir(seg.id.0.clone()),
            })
            .collect();
        if hops.is_empty() {
            return Some(crate::ref_resolve::ChainValue::Records(recs));
        }
        // Flat-map resolve_chain over the initial record set.
        let mut all_records: Vec<Record> = Vec::new();
        let mut all_scalars: Vec<String> = Vec::new();
        for rec in &recs {
            match crate::ref_resolve::resolve_chain(rec, &hops) {
                Some(crate::ref_resolve::ChainValue::Records(rs)) => all_records.extend(rs),
                Some(crate::ref_resolve::ChainValue::Scalars(ss)) => all_scalars.extend(ss),
                None => {}
            }
        }
        if !all_scalars.is_empty() {
            Some(crate::ref_resolve::ChainValue::Scalars(all_scalars))
        } else {
            Some(crate::ref_resolve::ChainValue::Records(all_records))
        }
    }
