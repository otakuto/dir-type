...
use dir_lint_expr::{ExprEntry, ExprMatcher};
...
pub(super) fn fetch_produced(
    ...
    let mut out = crate::record_map::RecordMap::new();
    ...
    for alt in alts {
        for child_dir in &tree.dirs {
                ...
                super::super::matcher::entry_matches_name(alt, &child_dir.name, scope, true)
            {
                ...
                if seen.insert(key) {
                    out.entry(fetch_id.clone())
                        .or_default()
                        .push(crate::value::Record {
...
pub(super) fn eval_fetch(
    ...
    for (id, records) in fetch_sets {
        work_scope.bind_env(crate::node_id::NodeKind::Fetch, id.clone(), records.clone());
