...
pub(super) fn lift_tag_from_child_out(child_out: &crate::record_map::RecordMap) -> Option<String> {
...
pub(super) fn dominant_use_rule(entries: &[ExprEntry]) -> Option<&dir_lint_yaml::RuleName> {
    ...
    for (idx, (entry, entry_scope, origin, _)) in expanded.iter().enumerate() {
        ...
        let mut interval = match entry.count {
            ...
            Quant::Default => {
                ...
                dir_lint_expr::Interval::at_least(1)
            }
        };
...
fn count_entry_name(entry: &ExprEntry, scope: &Scope) -> String {
    ...
    use crate::name_matcher::{MatchKind, ResolvedMatcher};
