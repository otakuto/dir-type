...
use dir_lint_expr::{ExprEntry, ExprMatcher, ExprRule, Quant};
...
pub(super) fn eval_choice(
    ctx: &mut EvalContext,
    group_id: &dir_lint_yaml::EntryId,
    alternatives: &[ExprEntry],
    min: usize,
    max: Option<usize>,
    source_path: Option<String>,
    work_scope: &mut Scope,
    produced: &mut crate::record_map::RecordMap,
) {
    ...
    for (alt_idx, alt) in alternatives.iter().enumerate() {
        ...
        if matches!(
            ...
            let mut alt_sink = crate::record_map::RecordMap::new();
        ...
        // the caller).
        work_scope.push();
        let mut alt_produced = crate::record_map::RecordMap::new();
        eval_entries(
            ctx,
            std::slice::from_ref(alt),
            work_scope,
...
pub(crate) fn alternative_realizes(
    ...
    let mut trial_produced = crate::record_map::RecordMap::new();
    eval_node_chained(
        tree,
        subtree,
        scope,
