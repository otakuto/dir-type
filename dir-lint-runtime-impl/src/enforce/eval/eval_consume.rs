...
use dir_lint_expr::{ExprEntry, ExprMatcher, ExprSubtree};
...
use crate::visible_ids::collect_visible_ids;
...
pub(super) fn eval_consume(
    ...
    sink: &mut crate::record_map::RecordMap,
) -> Vec<(
    std::collections::HashMap<String, String>,
    crate::record_map::RecordMap,
)> {
    ...
    if is_dir {
        ...
        if use_map_reduce {
            ...
            struct DescendJob {
                ...
                child_scope: crate::env::Scope,
                child_rule_owned: String,
                child_rule_chain: Vec<String>,
                inline_entries: Vec<dir_lint_expr::ExprEntry>,
            }
                ...
                if entry.id.is_some() {
                    for (k, v) in &captures {
                        child_scope.bind_lex(
                            ...
                            k.clone(),
                            crate::value::Value::Scalar(v.clone()),
                        );
                    }
                }
            ...
            struct DescendOut {
                errors: Vec<dir_lint_error::LintError>,
                dirs: Vec<dir_lint_runtime::DirTrace>,
                produced: crate::record_map::RecordMap,
                captures: std::collections::HashMap<String, String>,
            }
            ...
            let run_job = |job: DescendJob| -> DescendOut {
                ...
                let mut task_produced = crate::record_map::RecordMap::new();
            ...
            for (dir_idx, child_dir) in ctx.tree.dirs.iter().enumerate() {
                ...
                let inline_entries: &[ExprEntry] = match subtree {
                    ...
                    ExprSubtree::Leaf => {
                        ...
                        consumed.push((captures, crate::record_map::RecordMap::new()));
                        continue;
                    }
                };
                let mut child_scope = candidate.scope.clone();
                ...
                let mut child_produced = crate::record_map::RecordMap::new();
                eval_node_traced_chained(
                    child_dir,
                    inline_entries,
                    &child_scope,
