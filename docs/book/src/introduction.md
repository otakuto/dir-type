...
The foundational concept behind dir-lint is that forest. Rather than validating a single tree, dir-lint treats a *collection* of trees as the typed unit.
...
`dir-lint` is a CLI tool written in Rust. It reads a YAML rule file and validates an actual directory tree against it, reporting any discrepancy as a diagnostic message. It never looks inside files — it focuses exclusively on **structure**.

By analogy with a type system, `.dir-lint.yaml` is the "type definition file" for your directory structure. `dir-lint check` runs the type check and reports errors with error codes and line numbers when something does not match.

The essential value of dir-lint is that it lets you turn directory-structure conventions — previously buried in a corner of CONTRIBUTING.md, an old Slack thread, or someone's memory — into **an executable specification written in `.dir-lint.yaml`**. Tacit knowledge becomes a versioned type definition that anyone can read and verify.
