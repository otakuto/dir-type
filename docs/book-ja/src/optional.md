...
```yaml
{{#include ../../../tutorials/optional/.dir-type.yaml}}
```
...
このチュートリアルのディレクトリには `extra.txt` は存在するが、`opt.txt` は存在しない。`opt.txt` はルールに宣言されているにもかかわらず、`dir-type check` は成功する。これが `optional: true` の意味を端的に示している。存在すれば `::` で指定した内容に従って検証され、存在しなければ何も起きない。
