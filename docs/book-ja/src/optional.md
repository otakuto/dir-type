# `optional` — 存在してもしなくてもよい

`optional: true` を付けると、そのエントリは存在しなくても許可される。存在した場合は `::` で指定した内容に従って検証される。

`optional:` を使わない場合、`file:` や `dir:` エントリはすべて必須扱いになる。省略可能なエントリを宣言したいときに使う。
...
`optional: true` を付けたエントリは「存在したらそのエントリとして受理する」という意味を持つ。宣言されていない名前のファイルがディレクトリに存在した場合は、`optional:` の有無にかかわらず `LT001 undeclared path` になる。あくまでも「宣言されているが存在しなくてもよい」という意味だ。
